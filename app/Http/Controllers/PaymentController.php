...
{
    public function notification(Request $request) // .. 1
    {
        ...
        if ($paymentStatus && $payment) {
            DB::transaction(
                function () use ($order, $payment) {
                    if (in_array($payment->status, [Payment::SUCCESS, Payment::SETTLEMENT])) { // .. 2
...
// p: clue 1
// method ini kita tidak pakai csrf token
// sudah kita setting di app/Middleware/VerifiyCsrfToken

// p: clue 2
// jike status payment nya sukses atau settlement
// maka kita ganti status nya menjadi paid dan confirmed
